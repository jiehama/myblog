...
def userlogin(request):
    ...
    if user is not None:
        ...
        return redirect(request,'error.html',{'message':'用户名或密码不正确'})'''
...
def send_verification_code(request):
    ...
    if email != '':
        ...
        # 发送邮件
        send_mail('5tigher绑定邮箱','您的验证码：%s'% code,'jiehama2008@163.com',[email],fail_silently=False)
        data['status'] = 'SUCCESS'
    else:
        data['status'] = 'ERROR'
    return JsonResponse(data)
